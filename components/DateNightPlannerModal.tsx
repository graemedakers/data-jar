                    ...
                    {!itinerary && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-2">
                                <label className="text-sm text-slate-400 font-medium">When?</label>
                                <div className="relative">
                                    <Calendar className="absolute left-3 top-3 w-4 h-4 text-slate-500" />
                                    <input
                                        type="date"
                                        className="w-full bg-slate-800 border-slate-700 rounded-xl py-2.5 pl-10 pr-4 text-white focus:ring-2 focus:ring-pink-500/50 outline-none transition-all placeholder:text-slate-600 [color-scheme:dark]"
                                        value={targetDate}
                            ...
                            <div className="space-y-2">
                                <label className="text-sm text-slate-400 font-medium">Where?</label>
                                <div className="relative">
                                    <MapPin className="absolute left-3 top-3 w-4 h-4 text-slate-500" />
                                    <input
                                        type="text"
                                        placeholder="City or Neighborhood"
                                        className="w-full bg-slate-800 border-slate-700 rounded-xl py-2.5 pl-10 pr-4 text-white focus:ring-2 focus:ring-pink-500/50 outline-none transition-all placeholder:text-slate-600"
                            ...
                            <div className="space-y-2 md:col-span-2">
                                <label className="text-sm text-slate-400 font-medium">Budget</label>
                    ...
                    {itinerary && (
                        <div className="space-y-6 animate-in fade-in zoom-in-95 duration-500">
                            ...
                            <a
                                ...
                                className="block w-full py-3 bg-blue-500/10 hover:bg-blue-500/20 text-blue-300 hover:text-blue-200 border border-blue-500/20 rounded-xl text-center font-medium transition-colors flex items-center justify-center gap-2 mb-6"
                            >
                                ...
                                {itinerary.schedule.map((item, idx) => {
                                    ...
                                    return (
                                        <div key={idx} className="relative">
                                            {/* Timeline Dot */}
                                            <div className="absolute -left-[41px] top-0 w-6 h-6 rounded-full bg-slate-800 border-2 border-pink-500 flex items-center justify-center z-10">
                                                <div className="w-2 h-2 rounded-full bg-pink-500" />
                                            </div>

                                            <div className={`bg-slate-800/50 border border-slate-700 p-4 rounded-xl transition-colors group ${isEditing ? 'bg-slate-800 border-pink-500/50 ring-1 ring-pink-500/50' : 'hover:bg-slate-800'}`}>

                                                {isEditing && editForm ? (
                                                    <div className="space-y-3">
                                                        <div>
                                                            <label className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-1 block">Venue Name</label>
                                                            <input
                                                                value={editForm.venue_name}
                                                                onChange={e => setEditForm({ ...editForm, venue_name: e.target.value })}
                                                                className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white focus:border-pink-500 outline-none"
                                                            />
                                                        </div>
                                                        <div>
                                                            <label className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-1 block">Description</label>
                                                            <textarea
                                                                value={editForm.description}
                                                                onChange={e => setEditForm({ ...editForm, description: e.target.value })}
                                                                className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white focus:border-pink-500 outline-none h-20 resize-none"
                                                            />
                                                        </div>
                                                        <div>
                                                            <label className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-1 block">Booking / Access Link (URL)</label>
                                                            <input
                                                                value={editForm.booking_link}
                                                                onChange={e => setEditForm({ ...editForm, booking_link: e.target.value })}
                                                                className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-sm text-blue-300 focus:border-pink-500 outline-none"
                                                            />
                                                        </div>
                                                        <div>
                                                            <label className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-1 block">Address</label>
                                                            <input
                                                        ...
                                                        <div className="flex gap-2 pt-2">
                                                            ...
                                                            <Button size="sm" onClick={handleCancelEdit} variant="ghost" className="text-slate-400 hover:text-white h-8">
                                                    ...
                                                    <>
                                                        <div className="flex justify-between items-start mb-2">
                                                            <div>
                                                                <span className="inline-block px-2 py-0.5 rounded text-xs font-bold bg-pink-500/10 text-pink-300 mb-1 border border-pink-500/20">
                                                                    {item.time}
                                                                </span>
                                                            ...
                                                            <div className="flex gap-1">
                                                                ...
                                                                <button
                                                                    ...
                                                                    className="p-2 rounded-full bg-slate-700/50 text-slate-400 hover:text-white hover:bg-blue-500/20 hover:border-blue-500/30 border border-transparent transition-all"
                                                                    title="Edit Details"
                                                                >
                                                                ...
                                                                <button
                                                                    ...
                                                                    disabled={isRegenerating || isAdding}
                                                                    className="p-2 rounded-full bg-slate-700/50 text-slate-400 hover:text-white hover:bg-red-500/20 hover:border-red-500/30 border border-transparent transition-all"
                                                        ...
                                                        <div className="flex flex-col sm:flex-row gap-3 text-xs text-slate-400 mt-2">
                                                        ...
                                                        <div className="mt-3 pt-3 border-t border-slate-700/50 flex flex-wrap gap-4">
                                                            {item.booking_link && (
                                                                <a href={item.booking_link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-xs text-pink-300 hover:text-pink-200 hover:underline">
                            ...
                            <div className="flex gap-3 pt-4 border-t border-slate-800">
                                <Button
                                    ...
                                    className={`flex-1 font-bold ${isAdded ? "bg-green-500/20 text-green-400 border border-green-500/30" : "bg-white text-slate-900 hover:bg-slate-200"}`}
                                >
                                ...
                                <Button variant="outline" onClick={() => setItinerary(null)} className="border-slate-700 hover:bg-slate-800">
                                    Start Over
                                </Button>
                            </div>
                        </div>
