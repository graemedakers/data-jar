                ...
                <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto w-full">
                    ...
                    {ratings.length > 0 ? (
                        <div className="space-y-3">
                            <h4 className="text-xs text-slate-400 font-bold uppercase tracking-wider">Couple Ratings</h4>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                {ratings.map((rating: any, index: number) => (
                                    <div key={index} className="bg-slate-800/50 border border-slate-700 rounded-xl p-4">
                                        <div className="flex justify-between items-start mb-2">
                                            <div className="font-bold text-white text-sm">{rating.user?.name || "Partner"}</div>
                                        ...
                                        {rating.comment && (
                                            <div className="relative pl-3 border-l-2 border-slate-600">
                                                <p className="text-slate-300 text-xs italic">"{rating.comment}"</p>
                                            </div>
                                        )}
                                    </div>
                                ))}
                        ...
                        <div className="flex items-center gap-4 bg-yellow-500/10 border border-yellow-500/20 rounded-xl p-4">
                            <div className="text-yellow-500">
                                <Star className="w-8 h-8 fill-yellow-500" />
                            </div>
                            <div>
                                <div className="text-sm text-yellow-500 font-bold uppercase tracking-wider mb-0.5">Rating</div>
                                <div className="text-2xl font-bold text-white">{idea.rating}/5</div>
                            </div>
                            {idea.ratingComment && (
                                <div className="flex-1 border-l border-yellow-500/20 pl-4 ml-2">
                                    <Quote className="w-4 h-4 text-yellow-500/50 mb-1" />
                                    <p className="text-slate-300 text-sm italic">"{idea.ratingComment}"</p>
                                </div>
                            )}
                        </div>
                    ) : null}
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div className="bg-slate-800/50 rounded-lg p-3 text-center border border-slate-700">
                            ...
                            <div className="text-white font-medium">{idea.duration} hours</div>
                        </div>
                        <div className="bg-slate-800/50 rounded-lg p-3 text-center border border-slate-700">
                            <DollarSign className="w-4 h-4 text-green-400 mx-auto mb-1" />
                            <div className="text-xs text-slate-500 uppercase font-bold">Cost</div>
                            <div className="text-white font-medium">{idea.cost}</div>
                        </div>
                        <div className="bg-slate-800/50 rounded-lg p-3 text-center border border-slate-700">
                            ...
                            <div className="text-white font-medium capitalize">{idea.activityLevel?.toLowerCase()}</div>
                        </div>
                        <div className="bg-slate-800/50 rounded-lg p-3 text-center border border-slate-700">
                            ...
                            <div className="text-white font-medium">{idea.indoor ? 'Indoor' : 'Outdoor'}</div>
                        </div>
                    </div>
                    ...
                    {(idea.details || idea.notes) && (
                        <div className="space-y-4">
                            {idea.notes && (
                                <div>
                                    <h4 className="text-sm text-slate-400 font-bold uppercase tracking-wider mb-2">Notes</h4>
                                    <p className="text-slate-300 bg-slate-800/50 rounded-xl p-4 border border-slate-700 leading-relaxed whitespace-pre-wrap">
                                        {idea.notes}
                                    </p>
                                </div>
                            )}
                            ...
                            {idea.details && (
                                <div>
                                    <h4 className="text-sm text-slate-400 font-bold uppercase tracking-wider mb-2">Itinerary / Details</h4>
                                    <p className="text-slate-300 bg-slate-800/50 rounded-xl p-4 border border-slate-700 leading-relaxed whitespace-pre-wrap">
                                        {idea.details}
                                    </p>
                                </div>
